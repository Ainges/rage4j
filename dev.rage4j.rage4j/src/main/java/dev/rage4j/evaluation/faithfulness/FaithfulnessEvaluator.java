...
public class FaithfulnessEvaluator implements Evaluator
{
	...
	/**
	 * Asks the judge for its verdict on one claim: the structured answer first,
	 * and only when that answer cannot be parsed or carries no value, the
	 * plain-text variant read by {@link InferenceVerdictParser}. A structured
	 * answer that fails to parse is a formatting failure of the judge, not a
	 * verdict, so it is retried in the other format rather than counted; a
	 * transport failure (timeout, rejected request) is neither and propagates
	 * as it is — a second call would only repeat it. A {@code null} verdict is
	 * treated exactly like an unparsable one: a judge that answered
	 * {@code {"inferred": null}} gave no verdict, and counting it as
	 * {@code false} would be an established "not inferred" nobody established.
	 *
	 * @throws OutputParsingException
	 *             when the fallback answer carries no verdict either, or
	 *             contradicts itself — the claim then has no verdict, and the
	 *             sample no faithfulness value
	 */
