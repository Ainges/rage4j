...
public interface FaithfulnessBot
{
	...
	/**
	 * Structured verdict on one claim. The return type is a record, so LangChain4j derives a
	 * JSON schema from it: a model built with {@code response_format: json_schema} answers with
	 * the schema enforced, any other model gets the schema as an instruction appended to the
	 * user message and its answer is parsed as JSON. Either way the parser is LangChain4j's
	 * JSON parser, not its strict {@code Boolean} parser, which rejected {@code true⏎true} — the
	 * shape gpt-oss-120b writes in about a quarter of the samples.
	 */
	@SystemMessage("""
	...
	/**
	 * The same question as {@link #canBeInferred} with a plain-text answer, used by
	 * {@link FaithfulnessEvaluator} as the fallback when the structured verdict cannot be
	 * parsed or carries no value. The prompt is the one the metric used before structured
	 * output existed, unchanged, so a fallback verdict is comparable to earlier measurements.
	 * The text is read by {@link InferenceVerdictParser}, which accepts the word repeated and
